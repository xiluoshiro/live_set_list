...
当前仓库结构基线为 `B1__baseline_schema.sql`，后续 migration 为 V2~V19；V12 增加 `live_attrs.default_band_ids`，V13/V14 增加巡演和演出活动组聚合，V15 增加活动出演成员，V16 允许同一 session 保留多个已签发的 CSRF token hash，V17 为控制台 Setlist 完整集合更新授予窄范围删除权限，V18 增加 Live 状态与正式改期历史，V19 恢复四张完整集合关系表的窄范围删除授权。目标数据库的实际版本仍应通过 `flyway info` 确认。
