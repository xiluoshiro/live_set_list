# Flyway Scaffold

本目录用于存放当前项目的 Flyway 配置、baseline 与后续版本化 SQL。

建议使用方式：

1. 先阅读 [docs/design/flyway.md](D:/Code/PythonCode/5%20LiveSetList/docs/design/flyway.md)
2. 本地可直接使用 `flyway.toml` 连接 Docker 迁移目标库
3. 如需提交模板或切换环境，再参考 `flyway.toml.example`
4. `infra/postgres` 提供 Docker PostgreSQL 的 compose 和 env 配置
5. 当前默认先操作测试库 `live_statistic_test`
6. 将结构变更写入 `sql/V...sql`

修改表结构时：

1. 先在 pgAdmin 中试验 SQL
2. 将正式变更写入新的 `sql/V...sql`
3. 运行：

```powershell
flyway -configFiles=backend/db/flyway/flyway.toml validate
flyway -configFiles=backend/db/flyway/flyway.toml migrate
```

4. 验证接口与页面
5. 不要修改已执行过的 `V...sql`

`flyway.toml` 是本地配置且不提交；GitHub Actions 会从 `flyway.toml.example` 生成 CI 临时配置，出包脚本会再次显式排除它。生产 prepare 发现 `sql/` 与当前 release 不同时不会直接部署，而是要求单独的 `migrate` 阶段生成 root-only attestation；第二个 `deploy` 阶段只有在归档 SHA-256、SQL 树 hash 和生产 Flyway version 全部匹配时才会切换应用。详见 [docs/production-deployment-runbook.md](../../../docs/production-deployment-runbook.md)。

目录说明：

- `flyway.toml`
  - 本地 Docker 迁移目标库配置
- `flyway.toml.example`
  - Flyway 项目配置模板
- `sql/B1__baseline_schema.sql`
  - 当前 baseline migration
- `sql/V2__...` 到 `sql/V12__...`
  - 当前已经落地的认证、收藏、权限、控制台、`live_type` 与默认 Band 等版本化迁移
- `../postgres/init/010-create-flyway-role.sh`
  - PostgreSQL 容器初始化时创建 Flyway / app / test admin 角色并授予相应权限
