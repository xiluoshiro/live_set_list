-- 01: fill reviewed Venue IANA values and COMMIT. Run before 02.
-- Pure SQL. Existing matching values are a no-op; conflicting values abort the transaction.
-- Does not change revisions, verification timestamps, Live snapshots or name history.
BEGIN;
...
BEGIN
    IF EXISTS (SELECT 1 FROM public.venue_list v JOIN _venue_targets t USING(id)
               WHERE v.timezone_id IS NOT NULL AND v.timezone_id IS DISTINCT FROM t.timezone_id) THEN
        RAISE EXCEPTION 'Existing Venue timezone conflicts with reviewed IANA; no rows changed';
    END IF;
END
$guard$;
CREATE TEMP TABLE _changed ON COMMIT DROP AS
WITH changed AS (
    UPDATE public.venue_list v SET timezone_id=t.timezone_id
    FROM _venue_targets t WHERE v.id=t.id AND v.timezone_id IS NULL
    RETURNING v.id
) SELECT * FROM changed;
INSERT INTO public.audit_logs(user_id,action,resource_type,resource_id,payload_json)
SELECT NULL,'venue_location_update','venue',v.id::text,
       jsonb_build_object('source','venue_own_timezone_backfill_20260918',
                          'before',to_jsonb(b),'after',to_jsonb(v))
FROM public.venue_list v JOIN _changed c USING(id) JOIN _before_venues b USING(id);
DO $guard$
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.venue_list v FULL JOIN _before_venues b USING(id)
        LEFT JOIN _venue_targets t ON t.id=v.id
        WHERE v.id IS NULL OR b.id IS NULL
           OR (t.id IS NULL AND to_jsonb(v) IS DISTINCT FROM to_jsonb(b))
           OR (t.id IS NOT NULL AND (v.timezone_id IS DISTINCT FROM t.timezone_id
               OR (to_jsonb(v)-'timezone_id') IS DISTINCT FROM (to_jsonb(b)-'timezone_id')))
    ) THEN
        RAISE EXCEPTION 'Unexpected Venue change beyond the timezone fill';
    END IF;
    IF EXISTS ((SELECT * FROM public.live_attrs EXCEPT ALL SELECT * FROM _before_lives)
               UNION ALL (SELECT * FROM _before_lives EXCEPT ALL SELECT * FROM public.live_attrs)) THEN
        RAISE EXCEPTION 'Live snapshots changed';
    END IF;
...
END
$guard$;
SELECT count(*) AS timezones_filled FROM _changed;
COMMIT;
