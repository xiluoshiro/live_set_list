...
def test_auth_and_search_contract(integration_test_client):
    ...
    search_payload = {"query": "Hall", "country_code": "JP", "language_code": "ja"}
    assert client.post("/api/console/geography/search", json=search_payload).status_code == 403
    with patch("app.routers.console_geocoding.geocode", return_value={"status": "not_found", "items": [], "message": None,
        ...
        result = client.post("/api/console/geography/search", json=search_payload, headers=headers)
    assert result.status_code == 200, result.text
    lookup.assert_called_once_with(query="Hall", country_code="JP", language_code="ja")
...
def test_resolution_is_read_only(integration_test_client, integration_admin_connection):
    ...
    payload = {"latitude": 22.3, "longitude": 114.1, "request_id": "draft-1", "parts": "timezone",
               "country_code": "HK", "language_code": "zh-HK"}
    ...
    with patch("app.routers.console_geocoding.geocode", return_value=address) as lookup:
        result = client.post("/api/console/geography/resolve", headers=headers, json={**payload, "parts": "address"})
    assert result.status_code == 200, result.text
    lookup.assert_called_once_with(latitude=22.3, longitude=114.1, country_code="HK", language_code="zh-HK")
