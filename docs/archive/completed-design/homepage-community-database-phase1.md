...
阶段 1 已落地，且后续已经继续推进到公共搜索、乐队浏览、演出资料筛选和全宽应用框架。本文中的“全部内容”“详情弹窗”是阶段 1 当时的名称与交互；当前 UI 分别使用“演出资料”和独立详情视图：
...
- Live 详情当前通过 `activeRow` 和 `getLiveDetail()` 维护为独立详情视图。
...
- 应用加载完成后默认停留在首页。
- 登录成功后仍建议回到首页，而不是强制跳到演出资料；除非登录是由收藏或受限入口触发。
...
- 最近 Live 标题点击复用 `setActiveRow(row)` 打开现有独立详情视图。
...
2. 演出资料继续调用 `getLives(page, pageSize)`。
