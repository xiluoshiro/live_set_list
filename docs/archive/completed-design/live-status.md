...
- 售票中、售罄、停售、现场票等票务状态。
- 草稿、发布、下架等内容管理状态。
- Tour 自身的聚合状态。活动组按日期范围显示未开始、进行中或已结束；全部子场次取消时显示已取消。
- 资料修正历史的公开展示。
...
| `today` | 进行中 | `live_date` 等于 Live 当地今天 |
| `past` | 已结束 | `live_date` 早于 Live 当地今天 |

`today` 的公开文案使用“进行中”，但它仍是日期级阶段：无法区分当天开演前、实际演出期间和散场后，不应解释为分钟级实时状态。
...
`status_note` 保存延期或取消说明。允许为空，因为历史资料可能只能确认状态而无法找到可靠原因；`scheduled` 的说明统一归一化为 `null`。公开端在延期或取消状态面板中展示该说明。
...
- 活动组摘要使用起止日期派生聚合阶段：首场日期前为“未开始”，起止日期范围内为“进行中”，末场日期后为“已结束”。全部子场次取消时优先显示“已取消”。活动组仍可进入详情查看各子场资料。
- 活动组卡片仍显示名称、日期或日期范围、Band 和原有收录场数；取消数以明确文字追加：单日活动为 `已收录 N 场 · 取消 X 场`，多日活动为 `已收录 D 日 · N 场 · 取消 X 场`。
- 活动组卡片始终保留详情入口；取消子场次在组内继续按单场规则处理。
...
- 普通历史列表不被大量“已结束”标签占满。
- 单日或多日活动组返回一张带聚合阶段标签的卡片；包含取消 Live 时继续显示 `取消 X 场`，仅全部子场次取消时使用取消状态。
