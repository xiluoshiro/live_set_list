...
3. 打开活动组，断言“演出资料”在桌面导航和移动抽屉中保持选中。
4. 断言公演入口按日期、开演时间、Live ID 排序，只显示短标题；切换后仍能看到选中 Live 自身的日期、时间和 setlist。
5. 若选中 Live 属于巡演，断言可从组内详情进入巡演资料。
6. 收藏组内全部 Live 后切换“仅收藏”，断言显示一个活动组项。
7. 取消其中一场收藏并重新进入“仅收藏”，断言活动组退化为仍收藏的单场 Live，不伪造整体收藏。
