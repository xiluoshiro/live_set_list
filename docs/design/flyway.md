...
- 当前仓库内版本化迁移已到 `V17__grant_setlist_management_delete.sql`
...
- `backend/db/flyway/sql/B1__baseline_schema.sql`
  - 当前 baseline 脚本
  - 已准备完成，作为版本 `1` 保留
- `backend/db/flyway/sql/V2__...` 到 `V17__...`
...
- 当前最新 migration 文件为 `V17__grant_setlist_management_delete.sql`
- 目标库执行 `flyway migrate` 后，应应用 `B1` 以及 `V2` 到 `V17`
