...
- 当前仓库内版本化迁移已到 `V12__add_default_band_ids_to_live_attrs.sql`
...
- `backend/db/flyway/sql/B1__baseline_schema.sql`
  - 当前 baseline 脚本
  - 已准备完成，作为版本 `1` 保留
- `backend/db/flyway/sql/V2__...` 到 `V12__...`
  - 当前已经落地的认证、收藏、权限、控制台、`live_type`、歌曲唯一键、成员数据清理与默认 Band 版本化迁移
...
- 当前最新 migration 文件为 `V12__add_default_band_ids_to_live_attrs.sql`
- 目标库执行 `flyway migrate` 后，应应用 `B1` 以及 `V2` 到 `V12`
