...
- 当前仓库内版本化迁移已到 `V16__allow_multiple_csrf_tokens_per_session.sql`
...
- `backend/db/flyway/sql/B1__baseline_schema.sql`
  - 当前 baseline 脚本
  - 已准备完成，作为版本 `1` 保留
- `backend/db/flyway/sql/V2__...` 到 `V16__...`
  - 当前已经落地的认证、收藏、权限、控制台、`live_type`、歌曲唯一键、成员数据清理、默认 Band、巡演聚合、演出活动组聚合、活动出演成员与多标签页 CSRF token 版本化迁移
...
- 当前最新 migration 文件为 `V16__allow_multiple_csrf_tokens_per_session.sql`
- 目标库执行 `flyway migrate` 后，应应用 `B1` 以及 `V2` 到 `V16`
