...
- 当前仓库最新 migration 为 `V35__remove_venue_merge.sql`；此处说明迁移文件状态，不代表已核验本地数据库执行版本。
...
本节以仓库实现为依据；下文原始设计和迁移阶段不代表所有条目均已交付。进度只按代码、本地数据及本地验证衡量，不以远程部署或发布为完成条件。最初进度盘点为只读代码检查；本次删除合并功能后已通过 `python scripts/run_checks.py functional`，并在测试库执行 V35 迁移及 Flyway validate。本地主库未执行迁移，本轮未做浏览器视觉验收。
...
| 5 | P2 | 删除名称兼容列 | 核对所有读写、脚本及种子依赖后，新增独立 migration 删除 `venue_list.venue`，移除兼容写入并验证名称读取契约 |

待核验事项单列，不算作已证实的开发缺口：本地主库迁移版本与完整数据质量、地图桌面／窄屏真实交互。已完成的自动检查和测试库迁移见上方进度说明；本地主库重复项筛查不能替代完整数据核验。
...
本地主库检查未发现合并指向，用户决定移除该功能。删除合并预览和执行路由、专用模型、列表合并参数、合并过滤及来源 ID 跳转。V35 删除合并字段并重建当前名称视图；V28/V29 作为已执行迁移保留不改。本次代码变更不执行本地主库迁移，合并也不再列入待实现项。
