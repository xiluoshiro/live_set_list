...
日期：2026-09-18。状态：设计方案，尚未实现。本方案承接现有 Venue 位置编辑、地图关联及 IANA 回填，不新增业务版本体系。
...
实现修改业务代码后按根目录要求以 `python scripts/run_checks.py functional` 作最终自动验证。当前仅新增设计文档，不修改应用、依赖、数据库或回填脚本，也不为文档运行功能套件。
