...
两阶段 migration 发布代码、VM root-owned 入口、deploy-only sudoers 和 GitHub repository secrets/variables/Environments 已完成配置。生产当前已确认到 V13；当前仓库已增加 V14~V19 migration，但在取得生产 `flyway info`、release state / attestation 和 health 证据前，不得把生产状态写成 V14 或更高版本。tag 存在仍不等价于生产验收。
