...
export type ConsoleBandLineupVersion = {
  ...
  transition_live_id?: number | null;
  note: string | null;
  members: string[];
...
export type ConsoleBandHistory = {
  ...
  current_name_version_id?: number;
  current_lineup_version_id?: number;
  initialized: boolean;
...
export type ConsoleBandLineupVersionPayload = {
  version_label: string;
  change_type: "addition" | "removal" | "replacement" | "correction";
  members: string[];
  valid_from: string;
  note: string | null;
  transition_live_id: number | null;
...
export type ConsoleBandTransitionLiveCandidate = {
  live_id: number;
  live_name: string;
  live_date: string;
};
...
export type MapCandidateSearch = {
  provider: MapProvider;
...
export const previewConsoleLocality = (id: number, payload: GeoLocalityUpdate) =>
  geographyRequest<LocalityPreview>(`/localities/${id}/preview`, "POST", payload);
export const saveConsoleLocality = (id: number, payload: GeoLocalityUpdate, csrf: string) =>
  geographyRequest<GeoLocality>(`/localities/${id}`, "PUT", payload, csrf);
...
export type GeographyQualityPage = {
  items: GeographyQualityItem[];
...
export type ConsoleLiveSetlistAppendPayload = {
  /** Read-only draft context used for confirmation/display; API serializers remove it. */
  band_lineup_contexts?: ConsoleLiveBandLineupContext[];
...
function consoleLookupQuery(q?: string, limit = 20, page?: number, bandId?: number): string {
  const query = new URLSearchParams({
    limit: String(limit),
  });
  ...
  if (bandId !== undefined) {
    query.set("band_id", String(bandId));
  }
  return query.toString();
}

export async function getConsoleSongs(
  q?: string,
  limit = 20,
  page?: number,
  bandId?: number,
...
export async function getConsoleBandTransitionLiveCandidates(
  bandId: number,
  liveDate: string,
): Promise<ConsoleBandTransitionLiveCandidate[]> {
  const query = new URLSearchParams({ live_date: liveDate });
  const response = await fetchWithTimeout(
    `${BASE_URL}/api/console/bands/${bandId}/transition-live-candidates?${query.toString()}`,
    undefined,
    { requestKind: "console_band_transition_live_candidates" },
  );
  return expectJsonResponse<ConsoleBandTransitionLiveCandidate[]>(response);
}
...
export async function appendConsoleLiveSetlist(
  ...
  const response = await fetchWithTimeout(
    `${BASE_URL}/api/console/lives/${liveId}/setlist`,
    {
      ...
      headers: jsonHeaders(csrfToken),
      body: JSON.stringify({
        setlist_rows: payload.setlist_rows.map(({ band_member: _legacyBandMember, ...row }) => ({
          ...row,
