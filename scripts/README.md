...
当前仓库 migration 已到 V19；生产数据库已有证据仍只确认到 V13。`v2026-07-18-001` 已完成 V12/V13 migration 和应用切换。后续带 SQL 变化的 tag 仍必须按 migration release 验收，不能因为 tag 已存在或测试库已经迁移就假定生产数据库已经升级。
